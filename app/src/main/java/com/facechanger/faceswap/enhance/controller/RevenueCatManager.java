...
public class RevenueCatManager {
    ...
    public static synchronized RevenueCatManager getInstance() {
        if (instance == null) {
    ...
    public void fetchOfferings(PurchaseListener listener) {
    ...
    public void purchasePackage(Activity activity, Boolean isFromPaywall, Package packageToPurchase, PurchaseListener listener) {
        Purchases.getSharedInstance().purchasePackage(activity, packageToPurchase, new PurchaseCallback() {
            ...
            public void onCompleted(@NonNull StoreTransaction storeTransaction,
                ...
                if (isFromPaywall) {
                    ...
                    ApiRepository.verifyInAppPurchase(productId, receiptData, transactionId, orderId,
                            new ApiRepository.VerificationCallback() {
                                @Override
                    ...
                    // Server-side verification
                    ApiRepository.verifyPurchase(productId, receiptData, transactionId, orderId,
                            new ApiRepository.VerificationCallback() {
                                @Override
                                public void onSuccess() {
                                    listener.onPurchaseVerified();
    ...
    public void restorePurchases(Activity activity, PurchaseListener listener) {
