 ...
 * </pre>
 */
public final class AppDialogController {
    ...
    public static Dialog showCoinsRequiredDialog(
            ...
            @Nullable OnDialogActionListener listener) {
     ...
     * {@link PhotoTipsPrefs}), the {@code onProceed} callback fires
    ...
    public static Dialog showServerBusyDialog(
            @NonNull Context context,
            @Nullable OnServerBusyListener listener) {
        ...
        for (int i = 0; i < stars.length; i++) {
            ...
            stars[i].setOnClickListener(v -> {
                ...
                for (int j = starIndex + 1; j < stars.length; j++) {
                    stars[j].setImageResource(R.drawable.ic_star_empty);
                }
    ...
    public static Dialog showInsufficientCoinsDialog(
            ...
            double cost,
            double balance,
            @Nullable OnCoinDialogListener listener) {
        ...
        tvCostInfo.setText("Required: " + CoinManager.formatCost(cost) + " " + costUnit);
        tvBalanceInfo.setText("Your balance: " + CoinManager.formatCost(balance) + " " + balanceUnit);

        // Configure button visibility based on AppSystem flags
        boolean showBuy = AppSystem.isFeatureEnabled(AppSystem.KEY_SHOW_BUY_COINS_OPTION);
        btnBuyCoins.setVisibility(showBuy ? View.VISIBLE : View.GONE);

        Log.e("StaticValue.APP_EXP", "" + GlobleMMKVManager.getInstance().getInt(StaticValue.APP_EXP, 1));
