 ...
 * </pre>
 */
public final class AppFaceSessionManager {

    private static volatile AppFaceSessionManager instance;

    @Nullable
    private AppFaceSplashDataResponse.Data sessionData;
    ...
    static {
        // AI Image / Generation
        registerActionKeys(AppFaceLoadingActivity.ACTION_AI_IMAGE, "edit_image", "ai_image", "aiimage");
        registerActionKeys(AppFaceLoadingActivity.ACTION_IMAGE_GENERATE, "image/generate", "image_generate", "imagegen");
        registerActionKeys(AppFaceLoadingActivity.ACTION_TEXT_TO_IMAGE, "image/generate", "text_to_image", "texttoimage");

        // Face Swap
        registerActionKeys(AppFaceLoadingActivity.ACTION_FACE_SWAP, "faceswap/basic", "face_swap", "faceswap");
        registerActionKeys(AppFaceLoadingActivity.ACTION_MULTI_FACE_SWAP, "faceswap/multi", "multi_face_swap", "multifaceswap");
        registerActionKeys(AppFaceLoadingActivity.ACTION_COUPLE_SWAP, "faceswap/couple", "couple_swap", "coupleswap");
        registerActionKeys(AppFaceLoadingActivity.ACTION_VIDEO_FACE_SWAP, "video/faceswap", "video_face_swap", "videofaceswap");

        // Editing / Processing
        registerActionKeys(AppFaceLoadingActivity.ACTION_REMOVE_BG, "background/remove", "remove_bg", "removebg");
        registerActionKeys(AppFaceLoadingActivity.ACTION_BG_REPLACE, "background/replace", "bg_replace", "bgreplace");
        registerActionKeys(AppFaceLoadingActivity.ACTION_UPSCALE, "upscale/pro", "upscale", "image_upscale");
        registerActionKeys(AppFaceLoadingActivity.ACTION_ENHANCE_GFPGAN, "enhance", "face_enhance", "face_restoration");
        registerActionKeys(AppFaceLoadingActivity.ACTION_ENHANCE_FACE, "enhance", "face_enhance", "face_restoration");

        // Misc Features
        registerActionKeys(AppFaceLoadingActivity.ACTION_HAIR_COLOR, "hair/color", "hair_color", "haircolor");
        registerActionKeys(AppFaceLoadingActivity.ACTION_HAIR_STYLE, "hair/style", "hair_style", "hairstyle");
        registerActionKeys(AppFaceLoadingActivity.ACTION_VIRTUAL_TRY_ON, "virtual-try-on", "virtual_try_on", "try_on");
    }
    ...
    private AppFaceSessionManager() { }

    /** Thread-safe singleton accessor. */
    @NonNull
    public static AppFaceSessionManager getInstance() {
        if (instance == null) {
            synchronized (AppFaceSessionManager.class) {
                if (instance == null) {
                    instance = new AppFaceSessionManager();
                }
            }
        }
        return instance;
     ...
     *
     * @param data The parsed {@link AppFaceSplashDataResponse.Data} from the splash API
     */
    public void initialize(@NonNull AppFaceSplashDataResponse.Data data) {
        this.sessionData = data;
        this.cachedCredits = data.getRemainingLimit();

        // Auto-configure the global API client with the auth token
        AppFaceApiClient.getInstance().setAuthToken(data.getToken());

        // Apply server-driven coin system flags
        AppFaceAppSystem.setFeatureEnabled(AppFaceAppSystem.KEY_COIN_SYSTEM_ENABLED, data.isCoinSystemEnabled());
        AppFaceAppSystem.setFeatureEnabled(AppFaceAppSystem.KEY_SKIP_COIN_CHECK_IF_ENOUGH, data.isBypassCoinCheckWhenSufficient());

        // Apply server-driven version flag
        AppFaceAppSystem.setFeatureEnabled(AppFaceAppSystem.KEY_IS_LATEST, data.isLatest());
        AppFaceAppSystem.setFeatureEnabled(AppFaceAppSystem.KEY_SHOW_COINS_DIRECT_PURCHASE, data.show_coins_direct_purchase());
    }
    ...
    public void clear() {
        sessionData = null;
        cachedCredits = 0.0;
        AppFaceApiClient.getInstance().setAuthToken(null);
    }
     ...
     */
    @Nullable
    public AppFaceSplashDataResponse.AppConfiguration getConfig() {
     ...
     */
    @Nullable
    public AppFaceSplashDataResponse.Data getSessionData() {
        return sessionData;
    }

    @NonNull
    public AppFaceSplashDataResponse.VideoFaceSwapConfig getVideoFaceSwapConfig() {
        if (sessionData != null && sessionData.getVideoFaceSwapConfig() != null) {
            return sessionData.getVideoFaceSwapConfig();
        }
        return AppFaceSplashDataResponse.VideoFaceSwapConfig.fromJson(null);
    }
     ...
     */
    public void updateCreditsFromResponse(@NonNull AppFaceFaceSwapResponse response) {
