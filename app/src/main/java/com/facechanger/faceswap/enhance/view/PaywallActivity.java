...
public class PaywallActivity extends AppCompatActivity {
    ...
    private void initViews() {
        ...
        if (APP_EXP == 1) {
            ...
            int IS_PRM_PRC_SHOW = GlobleMMKVManager.getInstance().getInt(StaticValue.IS_PRM_PRC_SHOW, 1);
    ...
    public void onBackPressed() {
        if (isFromSplash) {
            Intent intent = new Intent(PaywallActivity.this, MainActivity.class);
    ...
    private void setupListeners() {
        ...
        btnCta.setOnClickListener(v -> {
            ...
            if (subscriptionPackage != null) {
                showLoading(true);
                RevenueCatManager.getInstance().purchasePackage(this, true, subscriptionPackage, new PurchaseListener() {
                    @Override
                    ...
                    public void onPurchaseVerified() {
                        ...
                        runOnUiThread(() -> {
                            ...
                            SessionManager.getInstance().setPremium(true);
                            logFacebookSubscriptionEvent();
                            btnCta.postDelayed(() -> {
                                Intent intent = new Intent(PaywallActivity.this, SplashActivity.class);
                                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
                                startActivity(intent);
                                finish();
                            }, 1000);
    ...
    private void logFacebookSubscriptionEvent() {
        try {
            if (subscriptionPackage != null) {
                ...
                FacebookEventsManager.getInstance().logSubscriptionPurchased(productId, price, currencyCode);
            }
        } catch (Exception e) {
